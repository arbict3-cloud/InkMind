...
10. 开发过程中不要频繁自动同步 GitHub；只有用户明确要求“同步/上传/推到 GitHub”时再推送。
11. 各模块必须有“生成草稿 / 对话或手动精修 / 保存采纳”的概念，只有保存采纳后的内容才允许被下游模块引用。
...
- “创作流程”页面已接入后端模型 API，可按阶段选择的 provider/model 直接生成总纲、卷纲、章纲和正文
- 新增正式的卷目录基础：后端 `volumes` 表、章节 `volume_id`、卷 CRUD API、写作页卷/章节树形侧栏
- 分卷大纲阶段可以把文本解析成卷目录，写作页会显示空卷和未分卷章节
- 写作页章节侧栏支持把章节移动到指定卷或移出分卷
- 将“创作流程”页重构为八模块小说工作台：世界观、故事创意、角色设定、全书大纲、卷大纲、章节大纲、正文生成、策划分析
- 新工作台支持左侧流程导航、每阶段单独模型选择、草稿编辑、保存采纳、右侧已采纳上下文预览
- 世界观模块支持参考作品结构拆解后生成原创世界观；故事、角色、全书、卷、章节、正文、策划模块会按已采纳内容向下引用
...
- 卷内章节拖拽排序
- 每个创作阶段独立保存 provider 和 model 到后端，目前先存于浏览器本地
